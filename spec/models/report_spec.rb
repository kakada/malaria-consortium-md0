...
describe Report do
  ...
  describe "report_cases " do
      before(:each) do
        @village  = Village.make  :code => "1234567890"
        @village1 = Village.make :code => "1234567891"
        @village2 = Village.make :code => "1234567892"
        @village3 = Village.make :code => "1234567893"
        @village4 = Village.make :code => "1234567894"

        @v  = User.make :user_name => "vuser"  , :phone_number => "85560000", :place => @village
        @v1 = User.make :user_name => "vuser1" , :phone_number => "85560001", :place => @village1
        @v2 = User.make :user_name => "vuser2" , :phone_number => "85560002", :place => @village2
        @v4 = User.make :user_name => "vuser4" , :phone_number => "85560004", :place => @village4

        @h = User.make :user_name => "huser0" , :phone_number => "85570000", :place => @village.health_center
        @h1 = User.make :user_name => "huser1" , :phone_number => "85570001", :place => @village1.health_center
        @h2 = User.make :user_name => "huser2" , :phone_number => "85570002", :place => @village2.health_center

        VMWReport.make :sender_id => @v1.id,
                     :place_id => @v1.place.id,
                     :village => @village1,
                     :created_at => "2010-08-10",
                     :updated_at => "2010-08-10"

        HealthCenterReport.make  :sender_id => @h1.id,
                           :place_id => @h1.place.id,
                           :village => @village1,
                           :created_at => "2010-08-12",
                           :updated_at => "2010-08-12"
      end
      describe "for none reporting places" do
        describe "for village" do
          it "should return only villages that have user that have not been reported in the time frame" do
            places = Report.report_cases :place_type => "Village",
                                         :ncase => "0",
                                         :from => "2010-08-08",
                                         :to =>  "2010-08-13"

            places.count.should eq 3
            places.should eq [@village, @village2, @village4]
          end
          
          it "should return all villages that have user when report is not in the time frame" do
            places = Report.report_cases :place_type => "Village",
                                         :ncase => "0",
                                         :from => "2010-08-01",
                                         :to =>  "2010-08-08"

            places.count.should eq 4
            places.should eq [@village, @village1, @village2, @village4]
                                         
          end
        end

        describe "for health_center" do
          it "should return only health_centers that have user that have not been reported in the time frame" do
            places = Report.report_cases :place_type =>"HealthCenter",
                                         :ncase => "0",
                                         :from => "2010-08-08",
                                         :to =>  "2010-08-16"
           
            places.count.should eq 2
            places.should eq [@village.health_center, @village2.health_center]
          end

          it "should return all health center that have user and report is not in the time frame" do
            places = Report.report_cases :place_type =>"HealthCenter",
                                         :ncase => "0",
                                         :from => "2010-08-01",
                                         :to =>  "2010-08-07"
            ...
            places.count.should eq 3
            places.should eq [@village.health_center, @village1.health_center, @village2.health_center]
          end
        end
      end
      describe "for none reporting places" do 
        describe " for village " do
          it "should return only villages that have been reported in the time frame" do
               places = Report.report_cases :place_type => "Village",
                                         :ncase => "1",
                                         :from => "2010-08-01",
                                         :to =>  "2010-08-18"
               places = places.map{|place| place.place_id}
               places.should eq [@village1.id]

          end

          it "should return nothing if the report is not in the time frame" do
              places = Report.report_cases :place_type => "Village",
                                         :ncase => "1",
                                         :from => "2010-09-01",
                                         :to =>  "2010-09-18"

              places.should be_empty
          end
          
        end

        describe " for health center " do
          it "should return only health center that have been reported in the time frame" do
               places = Report.report_cases :place_type => "HealthCenter",
                                         :ncase => "1",
                                         :from => "2010-08-11",
                                         :to =>  "2010-08-18"

              places = places.map{|place| place.place.id}
              places.should eq [@h1.place.id]

          end

          it "should also return village in the health_center that have been reported in the time frame(village is located in healthcenter)" do
             places = Report.report_cases :place_type => "HealthCenter",
                                         :ncase => "1",
                                         :from => "2010-08-10",
                                         :to =>  "2010-08-15"
          ...
          it "should return only one place with the same health center " do
             places = Report.report_cases :place_type => "HealthCenter",
                                         :ncase => "1",
                                         :from => "2010-07-10",
                                         :to =>  "2010-09-15"
