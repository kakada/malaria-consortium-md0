  ...
  def map_report
    ...
    if(params[:id].blank?  || params[:id].to_i == 0)
        ...
        if(place.type == "Country")
          sql = " SELECT province.*, count(report.id) as total FROM places province INNER JOIN reports report" +
          ...
          sql = " SELECT od.*, count(report.id) as total FROM places od LEFT JOIN reports report " +
                " ON od.id = report.od_id WHERE od.parent_id = #{place.id} GROUP BY od.id ORDER BY total DESC "
        elsif(place.type == "OD" )
          sql = " SELECT hc.*, count(report.id) as total FROM places hc LEFT JOIN reports report " +
          ...
          sql = " SELECT village.*, count(report.id) as total FROM places village LEFT JOIN reports report " +
